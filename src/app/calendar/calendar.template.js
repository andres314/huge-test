...
export default ({ selectedYear, selectedMonth, selectedDay, selectedDate, inputDate, emptyDays, closed, filter }) => {
	const isDayLessThanToday = day => new Date(selectedYear, selectedMonth, day + 1).getTime() < new Date().getTime(),
				...
				isSelected = (day) => parseInt(selectedDay) === day && selectedDate.getMonth() === selectedMonth && selectedDate.getFullYear() === selectedYear ? 'selected' : '',
				isDisabled = (day) => {
					const dayToValidate = new Date(selectedYear, selectedMonth, day).getDay();
					return (filter === 'weekdays' && !weekdaysNum.includes(dayToValidate))
							|| (filter === 'weekends' && !weekendsNum.includes(dayToValidate))
	...
	return `
		...
		<div class="ui-calendar" ${closed ? 'hidden' : ''}>
			...
			${range(isLeapYear() ? 29 : daysInMonth[selectedMonth]).map(day => `
				<button class="day ${isSelected(day)}"
					${isDisabled(day)}>
					${day}
				</button>
			`).join('')}
